	...
	<c:choose>
		<c:when test="${not empty company.searchVal }">
			<c:set
				...
				var="listUrl" />
			<c:set
				value="/sub/company/put/${company.pageIndex}/${company.id}/${company.searchKey}/${company.searchVal}"
				var="setUrl" />
		</c:when>
		<c:otherwise>
			<c:set value="/sub/company/put/${company.pageIndex}/${company.id}"
				var="setUrl" />
		...
		<div class="page-content">
			...
			<div class="content-wrapper">
						...
						<div class="page-title">
							<h4>
								<i class="icon-arrow-left52 position-left"></i> <span
									class="text-semibold">참여기업관리</span> - 지점등록
							</h4>
						</div>
				...
				<form class="" action="/sub/company/put" method="post" id="postFrm"
					...
					<input type="hidden" name="parent_id" value="${ companyRes.id}"><input
